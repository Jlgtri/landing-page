...
const root = "./dist"; // Icon source file path.
const src = "./img/hero-isometric.svg"; // Icon source file path.
const dest = "./img/favicons"; // Output directory path.
const htmlBasename = "./indexxxx.html"; // HTML file basename.

// Configuration (see above in the README file).
const configuration = {
  path: dest, // Path for overriding default icons path. `string`
  appName: "EVOKX", // Your application's name. `string`
...
const response = await favicons(path.join(root, src), configuration);
await fs.mkdir(path.join(root, dest), { recursive: true });
await Promise.all(
  response.images.map(
    async image =>
      await fs.writeFile(path.join(root, dest, image.name), image.contents)
  )
);
await Promise.all(
  response.files.map(
    async file =>
      await fs.writeFile(path.join(root, dest, file.name), file.contents)
  )
);
await fs.writeFile(path.join(root, htmlBasename), response.html.join("\n"));
